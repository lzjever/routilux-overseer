...
vi.mock("@/lib/services/query-service", () => ({
  queryService: {
    breakpoints: {
      ...
      create: vi.fn((jobId: string, request: any) =>
        Promise.resolve({
          breakpoint_id: "bp3",
...
describe("useBreakpointStore", () => {
  ...
  describe("loadBreakpoints", () => {
    ...
    it("should handle API errors", async () => {
      vi.mocked(queryService.breakpoints.list).mockRejectedValueOnce(new Error("Network error"));
  ...
  describe("toggleBreakpoint", () => {
    ...
    it("should do nothing if breakpoint not found", async () => {
      ...
      await act(async () => {
        await result.current.toggleBreakpoint(
          "job1",
          "non-existent",
          "worker1",
          "http://localhost:8000"
