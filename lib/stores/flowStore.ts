...
export const useFlowStore = create<FlowState>((set, get) => ({
  ...
  loadFlows: async (serverUrl) => {
    ...
    try {
      const response = await queryService.flows.list();
      if (!response?.flows) {
...
function convertFlowToNodes(flow: FlowResponse, isEditable: boolean = false): Node[] {
  ...
  try {
    const nodes = Object.entries(flow.routines)
      .map(([id, routine]) => {
        if (!routine) {
    ...
    console.log(
      `convertFlowToNodes: converted ${nodes.length} nodes from ${Object.keys(flow.routines).length} routines`
    );
    return nodes;
  } catch (error) {
...
function convertFlowToEdges(flow: FlowResponse, isEditable: boolean = false): Edge[] {
  ...
  try {
    ...
    console.log(
      `convertFlowToEdges: converted ${edges.length} edges from ${flow.connections.length} connections`
    );
    return edges;
  } catch (error) {
