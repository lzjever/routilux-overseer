...
test.describe('Debugging Features', () => {
  ...
  test('should display routine nodes on canvas', async ({ page, server }) => {
    const flowData = {
      ...
      connections: [
        { from: 'routine1.output', to: 'routine2.input' }
      ]
    };
    ...
    if (flowResponse.ok) {
      ...
      if (jobResponse.ok) {
        ...
        if (isVisible) {
          ...
          const sharedDataPanel = page.locator('[data-testid="shared-data-panel"], [data-testid="data-panel"]');
  ...
  test('should handle failed job error display', async ({ page, server }) => {
    ...
    const flowData = {
      ...
      connections: [
        { from: 'source.output', to: 'error_gen.input' }
      ]
    };
    ...
    if (flowResponse.ok) {
      ...
      if (jobResponse.ok) {
        ...
        if (status.toLowerCase().includes('fail')) {
          ...
          const errorDisplay = page.locator('[data-testid="error-display"], [data-testid="job-error"]');
          await expect(errorDisplay).toBeVisible({ timeout: 5000 }).catch(() => {
